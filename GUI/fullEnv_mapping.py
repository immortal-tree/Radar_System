...
ax.set_position([0.19, -0.01, 0.6, 1.05])
r_max = 100.0
ax.set_ylim([0.0, r_max])
ax.set_xlim([0.0, 2*np.pi])
ax.grid(color='#00b140', alpha=0.5)
ax.set_rticks(np.linspace(0.0, r_max, 5))
ax.set_thetagrids(np.linspace(0.0, 360.0, 10))
angles = np.arange(0, 361, 1)
theta = angles * (2*np.pi / 180.0)
dists1 = np.ones((len(angles),)) * r_max
dists2 = np.ones((len(angles),)) * r_max
pols1, = ax.plot([], linestyle='', marker='o', markerfacecolor='g',
                markeredgecolor='g', markeredgewidth=1.0,
                markersize=3.0, alpha=0.9)
pols2, = ax.plot([], linestyle='', marker='o', markerfacecolor='g',
                markeredgecolor='g', markeredgewidth=1.0,
                markersize=3.0, alpha=0.9)
                ...
                if ser_bytes:
                    ...
                    if len(data) == 3:
                        ...
                        if angle % 5 == 0:
                           ...
                           pols1.set_data(theta, dists1)  # Update for the first distance
                           #fig.canvas.restore_region(axbackground)

                           pols2.set_data(theta, dists2)  # Update for the second distance
                           fig.canvas.restore_region(axbackground)
                           ax.draw_artist(pols1)  # Draw the updated pols
                           fig.canvas.blit(ax.bbox)
                           ax.draw_artist(pols2)  # Draw the updated pols2

                           line1.set_data(np.repeat((angle * (2*np.pi / 180.0)), 2),
